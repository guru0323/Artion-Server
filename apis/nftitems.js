...
router.post('/nftdetails', async (req, res) => {
  try {
    let newTk = new NFTITEM();
    newTk.contractAddress = req.body.contractAddress.toLowerCase();
    newTk.tokenID = req.body.tokenID;
    newTk.supply = req.body.supply;
    newTk.imageURL = req.body.imageURL;
    newTk.owner = req.body.owner.toLowerCase();
    newTk.symbol = req.body.symbol;
    newTk.royalty = req.body._royalty;
    newTk.createdAt = new Date();
    // let tokenUri = await getTokenUri(req.body.contractAddress, req.body.tokenID);
    // newTk.tokenURI = tokenUri ? tokenUri : 'https://';
    newTk.tokenURI = 'https://';
    newTk.tokenType = req.body.contractType;
    let isBanned;
    if(req.body.tokenType == 1155){
      isBanned = await is1155CollectionBanned(req.body.contractAddress);
    } else {
      isBanned = await is721CollectionBanned(req.body.contractAddress);
    }
